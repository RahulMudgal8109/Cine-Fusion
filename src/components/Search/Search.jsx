...
const Search = () => {
  ...
  const SearchCall = () => {
    ApiCall(`${url}`).then((Response) => {
     ...
     let arr =  Response.results.filter((element) => (element.media_type === "movie" || element.media_type === "tv"))
     //console.log(arr)
     setSearchData([...arr]);
  ...
  return (
    <div className="Search">
      <CommonSectionSearch data = {searchData} heading = {`Search Results For: ${searchval}`}  />
    </div>
  )
}
